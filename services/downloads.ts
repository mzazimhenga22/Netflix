...
let _dirty = false;
let _activeResumables: Record<string, any> = {};
...
/** Force-refresh cache from disk (used by UI auto-recovery). */
export const refreshMetadataCache = async (): Promise<DownloadItem[]> => {
  _cache = null;
  return loadMetadata();
};

/** Get the current in-memory cache without hitting disk (used by UI polling). */
export const getMetadataCache = (): DownloadItem[] => {
  return _cache || [];
};
...
export const downloadVideo = async (
  ...
  const downloadResumable = FileSystem.createDownloadResumable(
    ...
    (dp) => {
      ...
      try {
        DownloadServiceModule.updateProgress(title, progress);
      } catch (e) {}

      // Persist to disk every ~10% (was ~20%)
  ...
  // Check if initially paused
  if (newItem.isPaused) {
    console.log(`[DownloadService] ${downloadId} is initially paused, waiting for resume...`);
  }

  try {
    // Start Foreground Service
    try {
      DownloadServiceModule.startForeground(`Downloading: ${newItem.title}`);
    } catch (e) {}

    // If paused, we don't start the downloadAsync immediately
    // The UI or resumeDownload will trigger it
    if (newItem.isPaused) {
       return null; 
    }
    ...
    if (item) {
      ...
      // Check if we need to stop the foreground service 
      // (only if no other downloads are active)
      const afterItems = await loadMetadata();
      const hasActive = afterItems.some(i => i.status === 'downloading' && !i.isPaused);
      if (!hasActive) {
         try { DownloadServiceModule.stopForeground(); } catch (e) {}
      }
    }
};

export const togglePauseDownload = async (id: string) => {
  const items = await loadMetadata();
  ...
  try {
    if (item.isPaused) {
      item.isPaused = false;
      await saveMetadata(items);
      // Resume MP4 if active
      if (_activeResumables[id]) {
        _activeResumables[id].resumeAsync();
      } else if (item.status === 'downloading') {
        // Re-start if not in memory (app restart)
        resumeDownload(item);
      }
    } else {
      item.isPaused = true;
      await saveMetadata(items);
      // Pause MP4 if active
      if (_activeResumables[id]) _activeResumables[id].pauseAsync();
    }
  } catch (err) {
    console.error('[DownloadService] Toggle pause error:', err);
  }
};

/**
 * Re-initializes a download (e.g. after app restart)
 */
export const resumeDownload = async (item: DownloadItem) => {
  if (item.status !== 'downloading') return;
  console.log(`[DownloadService] Resuming ${item.id}...`);
  try {
    // If we have a saved stream URL, try to ping it to see if it's still alive
    let resolvedStream: VidLinkStream | undefined;
    if (item.streamUrl) {
      try {
        const ping = await fetch(item.streamUrl, { method: 'HEAD', headers: item.streamHeaders });
        if (ping.ok) {
           resolvedStream = { url: item.streamUrl, headers: item.streamHeaders || {} };
           console.log(`[DownloadService] Reusing existing stream URL for ${item.id}`);
        }
      } catch (e) {
        console.log(`[DownloadService] Existing stream expired for ${item.id}, will resolve fresh link.`);
      }
    }

    // Restart the download process 
    // This will fetch a fresh link if resolvedStream is not provided
    await downloadVideo(
      item.tmdbId,
      item.title,
      item.type,
    ...
    console.error(`[DownloadService] Failed to resume ${item.id}`, err);
    // Graceful fallback: If link fetch fails, revert from 'failed' to 'paused'
    // so the user can manually retry later without losing their progress.
    const items = await loadMetadata();
    const failedItem = items.find(i => i.id === item.id);
    if (failedItem) {
      failedItem.status = 'downloading';
      failedItem.isPaused = true;
      await saveMetadata(items);
    }
  }
};

export const initializeDownloads = async () => {
  try {
    // Small delay to ensure DB/filesystem is ready before blasting requests
    await new Promise(resolve => setTimeout(resolve, 3000));
    const items = await loadMetadata();
    const downloadingItems = items.filter(i => i.status === 'downloading' && !i.isPaused);
    
    for (const item of downloadingItems) {
      console.log(`[DownloadService] Auto-resuming interrupted download: ${item.id}`);
      resumeDownload(item);
    }
  } catch (err) {
    console.error('[DownloadService] Auto-resume failed', err);
  }
};
...
/**
 * Get the local URI for playback and format type — works for both completed and partial downloads.
 */
export const getPlaybackUri = (item: DownloadItem): { uri: string, type?: 'm3u8' | 'mp4' } | null => {
  if (item.status === 'completed' || canPreview(item)) {
     if (item.localUri.endsWith('.m3u8')) {
        return { uri: item.localUri, type: 'm3u8' };
     }
     return { uri: item.localUri, type: 'mp4' };
  }
  return null;
};
...
async function downloadHlsVideo(
  ...
  url: string,
  headers: Record<string, string>,
  localUri: string, // now this should be forced to .m3u8 to identify it later
  tmdbId: string,
  title: string,
  type: 'movie' | 'tv',
  image: string,
  ...
  try {
    const segmentsDir = `${actualLocalUri}_segments/`;
    ...
    if (existingIdx > -1) {
       ...
       newItem.progress = items[existingIdx].progress;
       newItem.totalSize = items[existingIdx].totalSize;
    ...
    try {
      DownloadServiceModule.startForeground(`Downloading: ${title}`);
    } catch (e) {}

    // 4. Download segments in batches to avoid overwhelming the bridge/memory
    const batchSize = 2; // Reduced from 3 to 2 for lighter memory footprint
    let downloadedCount = 0;
    let totalSizeAccumulated = 0;

    // Fast-count already existing segments right away for progress
    for (let i = 0; i < segmentUrls.length; i++) {
        const segTarget = `${segmentsDir}seg_${i}.ts`;
        const stat = await FileSystem.getInfoAsync(segTarget);
        if (stat.exists) {
            downloadedCount++;
            if ('size' in stat) {
                totalSizeAccumulated += (stat as any).size as number;
            }
        }
    }

    for (let i = 0; i < segmentUrls.length; i += batchSize) {
      // Check for pause state before starting a new batch
      let currentItems = await loadMetadata();
      let currentItem = currentItems.find(it => it.id === downloadId);
      while (currentItem?.isPaused) {
        await new Promise(resolve => setTimeout(resolve, 2000)); // Poll every 2s
        currentItems = await loadMetadata();
        currentItem = currentItems.find(it => it.id === downloadId);
        if (currentItem?.status === 'failed') return; // Cancelled
      }
      ...
      await Promise.all(batch.map(async (segUrl, idx) => {
        ...
        try {
          ...
          if (stat.exists && stat.size > 0) {
             // We tracked its count/size in the fast-count loop above. Just skip network.
             return;
          }
          ...
          // Save every 40 segments (was 20) to reduce I/O pressure
          if (downloadedCount % 40 === 0 && _cache) await saveMetadata(_cache);
        } catch (err) {
    ...
    await FileSystem.writeAsStringAsync(actualLocalUri, localM3U8);
    ...
    if (item) {
     ...
     const endItems = await loadMetadata();
     const hasActive = endItems.some(i => i.status === 'downloading' && !i.isPaused);
     if (!hasActive) {
        try { DownloadServiceModule.stopForeground(); } catch (e) {}
     }
  }

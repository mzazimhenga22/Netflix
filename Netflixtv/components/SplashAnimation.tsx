...
export function SplashAnimation({ onFinish }: { onFinish: () => void }) {
  ...
  useEffect(() => {
    // Total animation time is approx 2.8s
  ...
  const zoomStyle = useAnimatedStyle(() => {
    const scale = interpolate(zoomProgress.value, [0, 1], [1, 60], Extrapolate.CLAMP);
    ...
    <Animated.View style={[styles.container, { opacity: containerOpacity }]}>
      <Animated.View style={[styles.nWrapper, zoomStyle]}>
        ...
        <Ribbon 
          ...
          rotation="-32deg"
          style={{ 
            left: 39, // Scaled from 19.5
            top: -30, // Scaled from -15
      ...
      <Animated.View style={[styles.lightBurst, useAnimatedStyle(() => ({
        ...
        transform: [{ scale: interpolate(zoomProgress.value, [0.4, 1], [0.5, 3]) }]
      }))]}>
        <LinearGradient

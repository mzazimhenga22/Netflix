...
(function() {
  ...
  window.fetch = function(...args) {
    ...
    return originalFetch.apply(this, args).then(async response => {
      if (!resolved && url.includes('/api/b/')) {
        apiBAttempts++;
        dbg('MATCH! /api/b/ found (attempt ' + apiBAttempts + '), reading response...');
        try {
          const clone = response.clone();
          const text = await clone.text();
          dbg('Response length: ' + text.length);

          // Empty response = VidLink doesn't have this content
          if (!text || text.length === 0) {
            dbg('Empty response from /api/b/ - content not available on VidLink');
            if (apiBAttempts >= 2) {
              // After 2 empty attempts, give up immediately
              resolved = true;
              window.ReactNativeWebView.postMessage(JSON.stringify({
  ...
  // Timeout — 30s for TV devices (slower CPUs + WebView)
  setTimeout(() => {
    if (!resolved) {
      dbg('TIMEOUT - no stream found in 30s');
      window.ReactNativeWebView.postMessage(JSON.stringify({
        type: 'VIDLINK_TIMEOUT'
      }));
    }

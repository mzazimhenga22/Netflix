...
function HomeHero({ title, description, imageUrl, logoUrl, item, top10, onPlay, onInfo }: HomeHeroProps) {
  ...
  useEffect(() => {
    if (item?.id) {
      const timer = setTimeout(() => setResolverEnabled(true), 2000);
      return () => clearTimeout(timer);
    }
  }, [item?.id]);

  // Handle resolved trailer
  const handleTrailerResolved = useCallback((stream: TrailerStream) => {
    console.log(`[Hero] 🎬 Trailer resolved for ${title}: ${stream.type} - ${stream.url.substring(0, 60)}...`);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.heroWrapper}>
        ...
        {/* Hidden WebView Trailer Resolver */}
        <TrailerResolver
          tmdbId={item?.id}
          mediaType={mediaType as 'movie' | 'tv'}
          onResolved={handleTrailerResolved}
          onError={(err) => console.warn(`[Hero] Trailer error: ${err}`)}
